...
export default function CommitteMemberScreen({ route, navigation }) {
  ...
  const canManageEvents = role === 'Chairman';
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.buttonsScroll} contentContainerStyle={styles.buttonsContainer} scrollEnabled={true} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          style={styles.button}
          onPress={() => navigation.navigate('EventCalendar', { councilId: Council, memberId: memberId, role: 'CommitteeMember' })}
        >
...
const styles = StyleSheet.create({
  ...
  // ── Scrollable buttons (was: buttonsContainer applied directly to ScrollView's `style`
  // with flex:1/justifyContent:'center'/bottom:20, which centers+clips instead of scrolling) ──
  buttonsScroll: {
    flex: 1,
    marginTop: 10,
  },
